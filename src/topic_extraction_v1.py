...
# 设置路径
input_file_path = os.path.join('data', 'processed_data', 'keywords_sbert_simplified.csv')
output_file_path = os.path.join('data', 'processed_data','topic_extracted_comments.csv')
topic_word_freq_file_path = os.path.join('data', 'topic_word_freq.csv')  # 保存主题词频的CSV文件路径
...
def main():
    ...
    # 确保 'cut_content' 列存在
    if 'cut_content' not in df.columns:
        raise ValueError("数据框中不存在 'cut_content' 列，请检查输入文件的列名。")

    # 对处理后的评论进行分词和停用词去除
    df['processed_content'] = df['cut_content'].apply(preprocess_text)
